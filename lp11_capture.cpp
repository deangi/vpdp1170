...
static void drain_policy() {
  ensure_fifo();
  if (!current_name[0]) {
    // Session may have been deferred when SD was missing at reset.
...
static void flush_all() {
  ensure_fifo();
  if (!current_name[0] || fifo.empty()) {
    fifo.clear();
    return;
  }
  uint8_t chunk[CHUNK_BYTES];
  while (!fifo.empty()) {
    size_t want = fifo.count();
    if (want > CHUNK_BYTES) want = CHUNK_BYTES;
    size_t n = pop_bytes(chunk, want);
    if (n == 0) break;
    if (!write_chunk(chunk, n)) {
      requeue(chunk, n);
      break;
  ...
  for (;;) {
    {
      ...
      ensure_mutex();
      if (session_mu && xSemaphoreTake(session_mu, pdMS_TO_TICKS(200)) == pdTRUE) {
        drain_policy();
...
void begin_session() {
  ...
  SD_FTP_StorageGuard guard;
  if (session_mu)
    xSemaphoreTake(session_mu, portMAX_DELAY);
