// LovyanGFX panel/touch config for Elecrow CrowPanel Advance 7"
// (ESP32-S3, SC7277 RGB 800x480, GT911). Based on Elecrow factory
// V1.3/V1.4/V1.5 HMI-bigInch7/LovyanGFX_Driver.h.
//
// Included only for VPDP_BOARD == VPDP_BOARD_CROWPANEL_7 (via gfx.h).
// CROW_PCLK_HZ comes from board_crowpanel.h; the guard keeps that value.
#pragma once
  ...
  LGFX(void) {
    {
      ...
      cfg.offset_x = 0;
      cfg.offset_y = 0;
      // Physical 180°: use panel offset only (do NOT also setRotation(2)).
...

    {
      ...
      // Match panel offset_rotation so getTouch() is in draw space.
      cfg.offset_rotation = 2;
